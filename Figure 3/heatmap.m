...
nsims=60; % 20, 20, 10, 10 <- sim batches used
...
allthetaes=-10:0.5:15;
allbetas=0.1:0.5:10;
allthetaas= -10:0.5:-0.1;


%the matrix that contains the % of time in Up state for each (thetaE,beta)
%couples explored
allperUP=zeros(numel(thetae),numel(beta),numel(thetaa));
sizebetas = numel(allbetas);
sizethetaas = numel(allthetaas);
mainmatrix = zeros(numel(thetae),numel(beta),numel(thetaa), numel(nsims));
disp("here")
parfor i=1:numel(allthetaes)
    thetae=allthetaes(i);
    disp("thetae")
    for j=1:sizebetas
        beta=allbetas(j);
        for kk=1:sizethetaas
            thetaa= allthetaas(kk);
            ...
            for k=1:nsims
                ...
                temp_per = percentUP(Y(2,newstart:end),thresh)
                per_UP=per_UP + temp_per;
                
                mainmatrix(i,j,kk,k)=temp_per;
            end
